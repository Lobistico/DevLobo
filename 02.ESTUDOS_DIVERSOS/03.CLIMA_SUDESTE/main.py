...
for i in range(0, 26):
    if root[i][0].text in codigos:
        ...
        cursor.execute('INSERT INTO previsao_tempo (codigo, atualizacao, temperatura, ar_umido, a_tempo) '
                       'VALUES (?, ?, ?, ?,?)', (dc_cid_cod[root[i][0].text], root[i][1].text, root[i][3].text,
                                                 root[i][6].text, root[i][5].text))
conexao.commit()
cursor.close()
conexao.close()
